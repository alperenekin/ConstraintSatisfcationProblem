from constraints.base_constraint import BaseConstraint


class Constraint4(BaseConstraint):  #  n(x=a) = n(y=b) + m

    def __init__(self, x, a, y, b, n, m):
        self.x = x
        self.a = a
        self.y = y
        self.b = b
        self.n = n
        self.m = m

    def constraintFunction(self, domains,index):
        for i in range(len(domains)):
            domainOfX = domains[i][self.x]
            if self.a in domainOfX:
                if len(domainOfX) == 1 and  self.a in domainOfX:
                    year = domains[i][self.n] # find year of a
                    if len(year) == 1:
                        for j in range(len(domains)):
                            domainOfY = domains[j][self.y] #find year of x
                            if self.b in domainOfY and len(domainOfY) == 1:
                                if i != j:
                                    second_year = domains[j][self.n]
                                    for year2 in second_year:
                                        if int(year[0]) != int(year2) + self.m:  # if year of first not greater than second then it is false
                                            return False
                                else: # they can not belong to same subject
                                    second_year = domains[j][self.n]
                                    if year[0] == second_year[0]:
                                        return False
        return True


